...
class APP(CTk):
    ...
    def display(self):
        ...
        self.current_code_entry = CTkTextbox(self.current_code_frame, width=500, height=600, font=("DejaVu Sans Mono", 12))
        ...
        self.convert_from_select = CTkComboBox(master=self.current_code_frame,
                values=['Python', 'Java', 'Kotlin', 'C++', 'C', 'C#', 'Haskel'])
        ...
        self.frame_label.grid(row=0, column=0, columnspan=3)
        self.converted_code_entry = CTkTextbox(self.converted_code_frame, width=500, height=600, font=("DejaVu Sans Mono", 12))
        ...
        self.pb = CTkProgressBar(master=self.converted_code_frame, orientation='horizontal', mode='intermediate', width=500, progress_color='green')
    ...
    def convert_handler(self):
        ...
        self.convert_code = threading.Thread(target=lambda: self.convert(self.code_to_convert, self.convert_from, self.convert_to))
    ...
    def convert(self, code_to_convert, convert_from, convert_to):
        openai.api_key = ("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")
        try:
